...
func _physics_process(delta):
  if ray_cast_2d.is_colliding():
    ...
    if ray_cast_collider != null:
      animated_sprite.scale.x = global_position.distance_to(ray_cast_2d.get_collision_point()) / 50
      animated_sprite.global_position = lerp(global_position, ray_cast_2d.get_collision_point(), 0.5)
